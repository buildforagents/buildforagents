...
def parse_readme(file_path="README.md"):
    ...
    for line in content.splitlines():
        ...
        if len(links_matches) > 1 and links_matches[1].group(1).lower() == "github":
            github_url = links_matches[1].group(2)
            desc_start = line.find(" - ", links_matches[1].end())
            if desc_start != -1:
                description = line[desc_start + 3:].strip()
            else:
                description = ""
        else:
            desc_start = line.find(" - ", links_matches[0].end())
            if desc_start != -1:
                description = line[desc_start + 3:].strip()
            else:
                description = ""
